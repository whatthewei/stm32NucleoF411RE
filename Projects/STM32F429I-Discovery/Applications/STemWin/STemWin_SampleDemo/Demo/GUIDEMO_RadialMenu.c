...
static void _cbDraw(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
    ...
    if (pPara->FinalMove) {
      ...
      GUI_DispStringHCenterAt((pPara->pBitmapItem + pPara->pItemInfo[pPara->NumItems - 1].Index)->pExplanation, 112, 178);
    }
    //
    // Draw frame surround the current item
    //
    GUI_SetColor(GUI_RED);
    GUI_DrawBitmap(&_bmRectRed_60x60, 82, 120);
    break;
  }
}
...
static void _RadialMenu(void) {
  ...
  ySizeWindow        = LCD_GetYSize() / 5 * 3;
  hDraw              = WM_CreateWindowAsChild(10, 30, xSizeWindow, ySizeWindow, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, _cbDraw, sizeof(pPara));
  //
