...
/* In High Speed mode, it is not possible to use DMA for writing/reading to/from Flash memory
  because the USB HS DMA is not connected (at product level) to this memory
  (refer to RM0090 reference manual for more details). 
  If DMA mode is enabled, an intermediate buffer must be used to interface between
  ...
  It is advised to disable DMA mode if it is not used for other interfaces. */
