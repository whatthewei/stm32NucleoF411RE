...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect,   "Video Player", ID_FRAMEWIN_INFO,    0,   0,   640, 439, 0, 0x64, 0 },
  { LISTVIEW_CreateIndirect, "Listview",     ID_VIDEO_LIST,       470, 05,  165, 380, 0, 0x0,  0 },   
  { SLIDER_CreateIndirect,   "Slider",      ID_PROGRESS_SLIDER,   05,  360, 435, 20,  0, 0x0,  0 },
}; 
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    hItem = BUTTON_CreateEx(360,  395, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_STOP_BUTTON);
    WM_SetCallback(hItem, _cbButton_stop);    
    
    hItem = BUTTON_CreateEx(152, 395, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
    ...
    hItem = BUTTON_CreateEx(295, 395, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next);
    
    hItem = BUTTON_CreateEx(87,  398, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_REPEAT_BUTTON);
    WM_SetCallback(hItem, _cbButton_repeat);     
      
    hItem = BUTTON_CreateEx(475, 398, 70,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_OPEN_BUTTON);
    WM_SetCallback(hItem, _cbButton_open); 

    hItem = BUTTON_CreateEx(535, 398, 70,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_ADD_BUTTON);
    WM_SetCallback(hItem, _cbButton_add);     
    
    hItem = BUTTON_CreateEx(600,  395, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_CLOSE_BUTTON);
    WM_SetCallback(hItem, _cbButton_close);    
    
    hItem = BUTTON_CreateEx(441,  357, 25,  25, pMsg->hWin, WM_CF_SHOW, 0, ID_FULL_SCREEN_BUTTON);
    WM_SetCallback(hItem, _cbButton_fullscreen);      
    
    hClient = WM_GetClientWindow(pMsg->hWin);
    WM_GetClientRectEx(hClient, &r);
    hFrame = WM_CreateWindowAsChild(r.x0 + 5, r.y0 + 5, r.x1 - 179, r.y1 - 85, hClient, WM_CF_SHOW, _cbVideoWindow , 0);
    ...
    hItem = WM_CreateWindowAsChild(05,
     ...
     WM_CreateWindowAsChild(639, 445, 1, 1, pMsg->hWin, WM_CF_SHOW | WM_CF_HASTRANS, _cbMediaConnection , 0); 
    break;
  ...
  case WM_PAINT:
    ...
    DrawRect3D(05, 386, 460, 50);
    DrawRect3D(470, 386, 165, 50); 
...
static void VideoDirectOpen(char *filename)
{
  ...
  VIDEOPLAYER_hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_GetDesktopWindowEx(1), 0, 41);
