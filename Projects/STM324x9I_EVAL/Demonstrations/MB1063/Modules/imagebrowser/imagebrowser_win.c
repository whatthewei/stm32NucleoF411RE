...
{
  { WINDOW_CreateIndirect,  "Window",           ID_WINDOW_0,       0,   0,  640, 439, 0, 0x0, 0 },
  { LISTBOX_CreateIndirect, "List",             ID_IMAGE_LIST,     445, 15, 180, 370, 0, 0x0, 0 }
};
...
static const GUI_WIDGET_CREATE_INFO _aSettingsDialogCreate[] =
{
  ...
  { TEXT_CreateIndirect,      "(seconds)",           ID_TEXT_2,        100, 50, 50,  20,  0, 0x0, 0 }, 
  { SPINBOX_CreateIndirect,   "Spinbox",             ID_SPINBOX_0,     30,  43, 60,  40,  0, 0x0, 0 }  
};


static uint8_t _acBuffer[1536];
FIL Image_File;
...
static int _DrawImage(FIL *hFile, uint8_t Enlarge, uint8_t type) 
{
  ...
  if(Enlarge == 0)
  {
    nx = 420000 / Info.XSize;
    ny = 370000 / Info.YSize;
  }
  else
  {
    nx = 640000 / Info.XSize;
    ny = 480000 / Info.YSize;
  }
...
static void ImageDirectOpen(char *filename)
{
  ...
  IMAGE_hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_GetDesktopWindowEx(1), 0, 41);
