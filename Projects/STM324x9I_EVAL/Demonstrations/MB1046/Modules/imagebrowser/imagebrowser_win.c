...
{
  { WINDOW_CreateIndirect,  "Window",           ID_WINDOW_0,       0,   0,  480, 248, 0, 0x0, 0 },
  { LISTBOX_CreateIndirect, "List",             ID_IMAGE_LIST,     306, 15, 160, 171, 0, 0x0, 0 }
};

static const GUI_WIDGET_CREATE_INFO _aSettingsDialogCreate[] =
{
  { FRAMEWIN_CreateIndirect,  "Settings",            ID_FRAMEWIN_0,    100, 50, 280, 120, 0, 0x0, 0 },
  { BUTTON_CreateIndirect,    "OK",                  ID_BUTTON_OK,     180, 5,  70,  30,  0, 0x0, 0 },
  { BUTTON_CreateIndirect,    "Cancel",              ID_BUTTON_CANCEL, 180, 50, 70,  30,  0, 0x0, 0 },
  { TEXT_CreateIndirect,      "Slide show timer : ", ID_TEXT_1,        30,  20, 100, 20,  0, 0x0, 0 },
  { TEXT_CreateIndirect,      "(seconds)",           ID_TEXT_2,        100, 45, 50,  20,  0, 0x0, 0 }, 
  { SPINBOX_CreateIndirect,   "Spinbox",             ID_SPINBOX_0,     30,  43, 60,  20,  0, 0x0, 0 }  
};
...
static int _DrawImage(FIL *hFile, uint8_t Enlarge, uint8_t type) 
{
  ...
  if(Enlarge == 0)
  {
    nx = 280000 / Info.XSize;
    ny = 168000 / Info.YSize;
  }
  else
  {
    nx = 480000 / Info.XSize;
    ny = 270000 / Info.YSize;
  }
...
static void _cbDialog(WM_MESSAGE * pMsg)
{
  ...
  case WM_INIT_DIALOG:
    ...
    imFrame = WM_CreateWindowAsChild(r.x0 + 15, r.y0 + 15, r.x1 - 191, r.y1 - 77, hItem, WM_CF_SHOW, _cbImageWindow, 0);
    
    /* Buttons initialization */
    
    hItem = BUTTON_CreateEx(88, 197, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_PREVIOUS_BUTTON);
    WM_SetCallback(hItem, _cbButton_previous);
    
    hItem = BUTTON_CreateEx(135, 190,  50,  50, pMsg->hWin, WM_CF_SHOW, 0, ID_SLIDE_BUTTON);
    WM_SetCallback(hItem, _cbButton_play);
    slideshow_state = OFF;
    
    hItem = BUTTON_CreateEx(197, 197, 35,  35, pMsg->hWin, WM_CF_SHOW, 0, ID_NEXT_BUTTON);
    WM_SetCallback(hItem, _cbButton_next);
    
    
    hItem = BUTTON_CreateEx(397, 200, 50,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_ADD_BUTTON);
    WM_SetCallback(hItem, _cbButton_add);
    
    hItem = BUTTON_CreateEx(325, 200, 50,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_OPEN_BUTTON);
    WM_SetCallback(hItem, _cbButton_open);
    
    hItem = BUTTON_CreateEx(268, 199, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_SETTINGS_BUTTON);
    WM_SetCallback(hItem, _cbButton_settings);
    
    hItem = BUTTON_CreateEx(20, 199, 30,  30, pMsg->hWin, WM_CF_SHOW, 0, ID_CLOSE_BUTTON);
 ...
 case WM_PAINT:
    DrawRect3D(15, 188, 288, 53);
    DrawRect3D(306, 188, 159, 53);
